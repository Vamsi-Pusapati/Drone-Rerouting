...
async def run():
    ...
    running_tasks = [print_telemetry_armed_task, print_telemetry_position_task]
    ...
    log("-- Go 0.2m North, 0m East, -0.2m Down within local coordinate system")
    prev_checkpoint = next_checkpoint
    ...
    log("Sending GPS spoofed location to move to different location  1.0m North, 0.0m East, -0.2m Down")
    next_checkpoint = [0.0, 0.2, -0.20]

    await drone.offboard.set_position_ned(PositionNedYaw(0.5, 0.0, -0.2, 0.0))
    await asyncio.sleep(4)
...
async def check_for_deviation(current_distance, previous_5_distance):
    ...
    if len(previous_5_distance) >=3:
        ...
        if deviation > DEVIATION_THRESHOLD:
            ...
            log("Redirecting to the Next Secure Checkpoint")
